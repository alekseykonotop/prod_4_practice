...
try:
    ...
    def callback(ch, method, properties, body):
        features, pair_id = json.loads(body)
        print(f"{pair_id}: Получен вектор признаков {body}")
        pred = regressor.predict(np.array(features).reshape(1, -1))

        # Опубликуем сообщение
        channel.basic_publish(exchange='',
                            routing_key='y_predict',
                            body=json.dumps([pred[0], pair_id]))
        print(f"{pair_id}: Предсказание с {pred[0]}, отправлено в очередь y_predict")
